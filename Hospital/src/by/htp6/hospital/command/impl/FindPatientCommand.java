...
public class FindPatientCommand implements Command {
	...
	private static final Logger log = LogManager.getLogger(FindPatientCommand.class);
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session == null) {
			...
			if (requestCurrentPage == null) {
				...
				currentPage = (sessionCurrentPage == null) ? 1 : Integer.parseInt(sessionCurrentPage);
			} else {
			...
			if (requestPatientsPerPage == null) {
				String sessionPatientsPerPage = "" + session.getAttribute(ParameterName.PATIENTS_PER_PAGE);
				patientsPerPage = (sessionPatientsPerPage == null) ? 10 : Integer.parseInt(sessionPatientsPerPage);
			} else {
			...
			if (searchData == null || "".equals(searchData)) {
				...
				try {
					...
					if (userType.equals(UserType.DOCTOR)) {
						...
						patientsCount = getPatientsCount.getPatientsCountByDoctorId(searchData, doctorId);
						patients = findPatientService.findPatientsByDoctorId(searchData, doctorId, offset,
								patientsPerPage);
						...
						patients = findPatientService.findPatients(searchData, offset, patientsPerPage);
