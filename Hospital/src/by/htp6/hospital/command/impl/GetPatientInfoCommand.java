...
/**
 * Команда предназначенная для получения полной информации о пациенте
 * 
...
public class GetPatientInfoCommand implements Command {
	...
	private static final Logger log = LogManager.getLogger(GetPatientInfoCommand.class);
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session == null) {
			...
			try {
				...
				List<Appointment> appointments = getAppointmentList.getAppointmentList(selectedPatient.getId(), status);
